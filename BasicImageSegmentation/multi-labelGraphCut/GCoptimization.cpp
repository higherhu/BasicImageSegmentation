...
GCoptimization::EnergyType GCoptimization::swap(int max_num_iterations)
{
	...
	try
	{
		while ( old_energy > new_energy && curr_cycle <= max_num_iterations)
		{
			...
			std::cout<<"current cycle:"<<curr_cycle<<", old energy:"<<old_energy<<", new energy"<<new_energy<<std::endl;
...
{
	SiteID _features,i,pix;
	SiteID n;

	for ( i = startY; i < endY; i++ )
		for ( _features = startX; _features < endX; _features++ )
		{
			pix = _features+i*m_width;
