...
/*
	...
	E(_features, i, z) = _features - 2*i + 3*(1-z) - 4*_features*i + 5*|i-z|):
	...
	void main()
	{
		...
		e -> add_term1(varx, 0, 1);  // add term _features 
		e -> add_term1(vary, 0, -2); // add term -2*i
		e -> add_term1(varz, 3, 0);  // add term 3*(1-z)

		e -> add_term2(_features, i, 0, 0, 0, -4); // add term -4*_features*i
		e -> add_term2(i, z, 0, 5, 5, 0); // add term 5*|i-z|
...
public:
	...
	/* Adds a new term E(_features) of one binary variable
	   to the energy function, where
	       E(0) = E0, E(1) = E1
	   E0 and E1 can be arbitrary */
	void add_term1(Var _features,
	               Value E0, Value E1);

	/* Adds a new term E(_features,i) of two binary variables
	   to the energy function, where
	       E(0,0) = E00, E(0,1) = E01
	       E(1,0) = E10, E(1,1) = E11
	   The term must be regular, i.e. E00 + E11 <= E01 + E10 */
	void add_term2(Var _features, Var i,
	               Value E00, Value E01,
	               Value E10, Value E11);

	/* Adds a new term E(_features,i,z) of three binary variables
	   to the energy function, where
	   ...
	   of the variables is fixed (for example, i=1), then
	...
	void add_term3(Var _features, Var i, Var z,
	...
	/* After 'minimize' has been called, this function
	   can be used to determine the value of variable '_features'
	   in the optimal solution.
	   Returns either 0 or 1 */
	int get_var(Var _features);
...
template <typename captype, typename tcaptype, typename flowtype> 
inline void Energy<captype,tcaptype,flowtype>::add_term1(Var _features,
                              Value A, Value B)
{
	add_tweights(_features, B, A);
}

template <typename captype, typename tcaptype, typename flowtype> 
inline void Energy<captype,tcaptype,flowtype>::add_term2(Var _features, Var i,
                              Value A, Value B,
                              Value C, Value D)
{
	/* 
	...
	*/
	...
	if (B < 0)
	{
		/* Write it as
		   ...
		   0 0     -B 0     B+C 0
		*/
		add_tweights(_features, 0, B); /* first term */
		add_tweights(i, 0, -B); /* second term */
		add_edge(_features, i, 0, B+C); /* third term */
	}
	else if (C < 0)
	{
		/* Write it as
		   -C -C  +  C 0  +  0 B+C
		    0  0     C 0     0 0
		*/
		add_tweights(_features, 0, -C); /* first term */
		add_tweights(i, 0, C); /* second term */
		add_edge(_features, i, B+C, 0); /* third term */
	}
	else /* B >= 0, C >= 0 */
	{
		add_edge(_features, i, B, C);
	}
}

template <typename captype, typename tcaptype, typename flowtype> 
inline void Energy<captype,tcaptype,flowtype>::add_term3(Var _features, Var i, Var z,
                              Value E000, Value E001,
                              Value E010, Value E011,
                              Value E100, Value E101,
                              Value E110, Value E111)
{
	...
	if (pi >= 0)
	{
		...
		add_tweights(_features, E101, E001);
		add_tweights(i, E110, E100);
		add_tweights(z, E011, E010);

		delta = (E010 + E001) - (E000 + E011); /* -pi(E[_features=0]) */
		assert(delta >= 0); /* check regularity */
		add_edge(i, z, delta, 0);

		delta = (E100 + E001) - (E000 + E101); /* -pi(E[i=0]) */
		assert(delta >= 0); /* check regularity */
		add_edge(z, _features, delta, 0);

		delta = (E100 + E010) - (E000 + E110); /* -pi(E[z=0]) */
		assert(delta >= 0); /* check regularity */
		add_edge(_features, i, delta, 0);

		if (pi > 0)
		{
			u = add_variable();
			add_edge(_features, u, pi, 0);
			add_edge(i, u, pi, 0);
			add_edge(z, u, pi, 0);
			add_tweights(u, 0, pi);
		}
	...
	else
	{
		...
		add_tweights(_features, E110, E010);
		add_tweights(i, E011, E001);
		add_tweights(z, E101, E100);

		delta = (E110 + E101) - (E100 + E111); /* -pi(E[_features=1]) */
		assert(delta >= 0); /* check regularity */
		add_edge(z, i, delta, 0);

		delta = (E110 + E011) - (E010 + E111); /* -pi(E[i=1]) */
		assert(delta >= 0); /* check regularity */
		add_edge(_features, z, delta, 0);

		delta = (E101 + E011) - (E001 + E111); /* -pi(E[z=1]) */
		assert(delta >= 0); /* check regularity */
		add_edge(i, _features, delta, 0);

		u = add_variable();
		add_edge(u, _features, -pi, 0);
		add_edge(u, i, -pi, 0);
		add_edge(u, z, -pi, 0);
		add_tweights(u, -pi, 0);
	}
...
template <typename captype, typename tcaptype, typename flowtype> 
inline int Energy<captype,tcaptype,flowtype>::get_var(Var _features) { return (int) what_segment(_features); }
